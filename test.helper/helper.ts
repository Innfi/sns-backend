...
class TestHelper {
  ...
  toLoadUserAccountInput(
    createInput: CreateUserAccountInput,
  ): LoadUserAccountInput {
    return {
  ...
  isValidTimeline(input: UserTimelineInput, timeline: IUserTimeline): boolean {
  ...
  newTimelineInputArray(userId: string, len: number): UserTimelineInput[] {
    ...
    for (let i = 0; i < len; i++)
      inputArray.push({ authorId: userId, text: 'sample text' });
  ...
  async writeTimelines(
    repo: TimelineRepository,
    inputArray: UserTimelineInput[],
  ) {
  ...
  hasMember(result: LoadRelationMembersResult, followerId: string): boolean {
    return (
      result.members!.findIndex((value: string) => value == followerId) >= 0
    );
  }
