...
describe('jwt', () => {
    const jwtObject: object = { secret: 'notsosafesecret' };
    ...
    it('current: check timestamp', () => {
        ...
        let futureToken = jwt.sign(
            {email: 'innfi@test.com', iat: Math.floor(Date.now() / 1000) + gap },
            jwtObject['secret'], { expiresIn: '10m'});
        let currentToken = jwt.sign(
            {email: 'innfi@test.com' },
            jwtObject['secret'], { expiresIn: '10m'});

        const futureDecoded: object = jwt.verify(futureToken, jwtObject['secret']) as object;
        const currentDecoded: object = jwt.verify(currentToken, jwtObject['secret']) as object;
